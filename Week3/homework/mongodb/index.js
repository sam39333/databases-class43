...
async function findEpisodesExercises(client) {
  ...
  const titleEpisode2Season2 = episode2Season2.title;
  console.log(
    `The title of episode 2 in season 2 is ${titleEpisode2Season2}`
  );

  // Find the season and episode number of the episode called "BLACK RIVER" [Should be: S02E06]
  const blackRiverEpisode = await collection.findOne({ title: "BLACK RIVER" });
  const seasonEpisodeBlackRiver = blackRiverEpisode.episode;
  console.log(
    `The season and episode number of the "BLACK RIVER" episode is ${seasonEpisodeBlackRiver}`
  );
...
async function updateEpisodeExercises(client) {
  ...
  const resultUpdateBushes = await collection.updateMany(
    { elements: "BUSHES" },
    { $set: { "elements.$": "BUSH" } }
  );
